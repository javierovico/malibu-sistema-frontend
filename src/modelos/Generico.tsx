...
type Editor<T> = {
    (nuevo?:T, original?:T): Promise<T|undefined>
}


export const useGenericModel = <Model extends Ideable, ModelSort extends string, ModelQuery>(url:string, page: number, perPage: number, editor?: Editor<Model>, sortBy?: ItemSorteado<ModelSort>[], itemsBusqueda?: Partial<ModelQuery>) => {
    ...
    const modelUpdate = useCallback((p: Model, borrar: boolean = false)=>{
        return new Promise<void>((res,rej) => {
            if (editor) {
                ...
                editor(borrar?undefined:p, modelOriginal)
                    .then((modelSubido)=>{
